...
from pml.blocks.algorithms import DifferentiableCostMinimizer
...
class DataStreamMonitoring(SimpleExtension, MonitoringExtension):
    ...
    def __init__(self, variables, data_stream, updates=None, **kwargs):
        ...
        self._evaluator = DatasetEvaluator(variables, updates)  # Theano function should be built in DatasetEvaluator
...
class TrainingDataMonitoring(SimpleExtension, MonitoringExtension):
    ...
    update.
    The monitored results are aggregated by epoch
    ...
    def do(self, callback_name, *args):
        ...
        if callback_name == 'before_training':
            if not isinstance(self.main_loop.algorithm,
                ...
                raise ValueError
            # Insert updates into the training progressing
            self.main_loop.algorithm.add_updates(
                self._buffer.accumulation_updates)
            self._buffer.initialize_aggregators()   # Set shared variable to be zero.
        else:
