...
class SogouSingleTaskDataset(SogouBaseDataset):
    ...
    def _map_for_train(self, raw_dataset):
        ...
        queries_per_user = np.array(
            [[np.array(list(set([self._get_token_index(token, self._token2index) for token in query])),
    ...
    def _map_for_test(self, raw_dataset):
        ids = np.array(raw_dataset['id'])
        queries_per_user = np.array(
            [[np.array(list(set([self._token2index[token] for token in query if token in self._token2index])),
